  ...
  useEffect(() => {
    const getCarById = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:3000/api/car/${vehicle.id}`);
        const data = await res.json();

        setVehicleData(data.data);
      } catch (error) {
        console.error("Error al traer el auto:", err);
      } finally {
        setLoading(false);
      }
    };
      ...
      >
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            {[
              { label: "Año", val: vehicleData.anio },
              {
                label: "Km",
                val: `${vehicleData.kilometraje?.toLocaleString("es-AR")} km`,
              },
              { label: "Estado", val: vehicleData.estado },
            ].map(({ label, val }) => (
              <div
                key={label}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              >
                ...
                {vehicleData.precio?.toLocaleString("es-AR")}
              </p>
            </div>
